...
<head>
    ...
    <title>Kelola Akun - Arsip Digital (Admin)</title>
</head>
<body>
    <div class="sidebar">
    ...
    <?php
        ...
        if(empty($_GET['status'])){
            ...
            // Pengambilan Data dari Database MySQL
            $sql = $conn->prepare("SELECT * FROM `admin`, jabatan WHERE admin.id_jabatan = jabatan.id_jabatan");
            $sql->execute();
            $data = $sql->fetchAll();
        } else if($_GET['status'] == 2) {
            // Pengambilan Data dari Database MySQL
            $sql = $conn->prepare("SELECT * FROM `pegawai`, jabatan WHERE pegawai.id_jabatan = jabatan.id_jabatan");
            $sql->execute();
            $data = $sql->fetchAll();
        } else if($_GET['status'] == 3) {
            // Pengambilan Data dari Database MySQL
            $sql = $conn->prepare("SELECT * FROM kepala_kantor, jabatan WHERE kepala_kantor.id_jabatan = jabatan.id_jabatan");
            $sql->execute();
            $data = $sql->fetchAll();
        }

        // Pengulangan(Looping) Data dari Database MySQL
        foreach ($data as $value){
            // Pengambilan Data dari Database MySQL
            $selec = $conn->prepare("SELECT * FROM user WHERE username='$value[username]'");
            $selec->execute();
            $select = $selec->fetch();
    ?>
                                        <tr>
                                            ...
                                            <td>
                                                <a href="edit.php?id=<?php echo $select['id_user'] ?>">
                                                <button class="btnlht" title="Edit"><i class='bx bxs-edit'></i></button></a>
                                                <a href="hapus.php?id=<?php echo $select['id_user'] ?>"
