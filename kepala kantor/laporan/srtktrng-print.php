...
<head>
    ...
    <title>Laporan Keseluruhan Surat Keterangan Tahun <?php echo $_GET['tahun'] ?> Bulan <?php echo $bln ?></title>
</head>
<body onload="window.print();">
    ...
    <h1>Laporan Keseluruhan Surat Keterangan<br>Tahun <?php echo $_GET['tahun'] ?> di Bulan <?php echo $bln ?></h1>
    <table class="tbl">
        <tr>
            <td width="20px">No.</td>
            <td>Jenis Surat</td>
    ...
    <?php
        ...
        $sql = $conn->prepare("SELECT * FROM surat_keterangan WHERE MONTH(tanggal_surat)='$_GET[bulan]'
        AND id_jenis_keterangan = '$_GET[jenis]' AND YEAR(tanggal_surat)='$_GET[tahun]'");
        $sql->execute();
        $select = $sql->fetchAll();
        // Perulangan(Looping) Data dari Database MySQL
        foreach($select as $value){
            // Pengambilan Data dari Database MySQL
            $jns = $conn->query("SELECT * FROM jenis_keterangan WHERE id_jenis_keterangan = '$_GET[jenis]'");
            $jns->execute();
            $jenis = $jns->fetch();
            ?>
            <tr>
                <td><?php echo $no++; ?></td>
                <td><?php echo $jenis['jenis_keterangan']; ?></td>
